...
```js
{lv:2,cat:"カテゴリ名",situation:"状況説明",q:"設問",options:["A","B","C","D"],answer:1,explain:"解説"}
```

**種別（トラック）について:** 入り口画面で「エンジニア基礎知識」か「案件特有の詳細」かを選ぶ。どちらに入るかは原則 `cat` 名から自動判定される（`trackOf()`）:
- `cat` が `基礎 / xxx` で始まる、または `用語理解` → **基礎知識**トラック
- それ以外の具体カテゴリ（`設計判断` `スコープ設計` `セキュリティ` 等）→ **案件特有**トラック

自動判定が実態と食い違うときだけ、明示の `track:"基礎"` / `track:"案件"` を足して上書きする（例: 具体カテゴリだが内容は案件非依存の一般知識、など）。基本は `cat` 名を規約どおり付ければ `track` は不要。
...
このカテゴリ名がそのまま入り口の種別（基礎知識／案件特有）の振り分けになる（上記「種別について」参照）ので、土台問題には必ず `基礎 /` を付ける。
